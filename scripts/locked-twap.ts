...
async function run(config: Config) {
  ...
  await lockingService.addLatestBlockNumber(endBlock);
}
...
async function parseLockedValue(
  ...
  for (const lock of lockedEventList) {
    ...
    const value = amount.gt(new BN("35000"))
      ? price.mul(amount).div(new BN(2))
      : price.mul(amount);
  ...
  const ethUsdcPool = new l2Config.web3.eth.Contract(
    SushiPoolABI as any,
    l2Config.ethUsdcPool
  );
  ...
  const ethPrice = new BN(usdcReserves)
    .mul(new BN(10).pow(new BN(12)))
    .div(new BN(ethReserves));
  console.log("ethPrice: " + ethPrice);
  const ethImoPool = new l2Config.web3.eth.Contract(
    SushiPoolABI as any,
    l2Config.ethImoPool
  );
  ...
  const imoPrice = new BN(ethInImoPool)
    .mul(ethPrice)
    .mul(new BN(1000))
    .div(new BN(ImoReserves));
  console.log("imoPrice: " + imoPrice);
  const LPContract = new l2Config.web3.eth.Contract(
    ERC20ABI as any,
    l2Config.ethImoLPToken
  );
  const totalLPStaked = await LPContract.methods
    .balanceOf(l2Config.ethImoStaking)
    .call();
  const totalLPStakedInUSD = new BN(totalLPStaked)
    .mul(new BN(43))
    .div(new BN(10).pow(new BN(18)));
  console.log("totalLPStakedInUSD" + totalLPStakedInUSD);
  const LPapr = TOTAL_PAYOUT.mul(imoPrice)
    .div(new BN(1000))
    .mul(new BN(100))
    .div(totalLPStakedInUSD)
    .div(new BN(10).pow(new BN(18)));
  ...
  const apr = TOTAL_PAYOUT.mul(new BN(4))
    .div(tvl)
    .mul(new BN(100))
    .mul(imoPrice)
    .div(new BN(1000));
